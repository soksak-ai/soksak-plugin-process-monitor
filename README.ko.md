...
선택한 environment에 등록된 owner가 공개하고 현재 project root와 같거나 그 하위인 `cwd`의
record만 표시합니다. 터미널이나 sidecar가 실제로 열려야 record가 생기며 browser view는 process를
만들지 않습니다. 폴링은 하지 않으며 Core가 live process event 표면을 제공한 뒤에만 추가합니다.
